...
- `GET /o/:token` always returns a 1×1 GIF (`image/gif`) with `Cache-Control: no-store`. Invalid tokens still return the GIF so clients do not show a broken image.
- `GET /c/:token` 302s to the stored original URL only if it is `http:` or `https:`. Other schemes are rejected (`invalid_destination`). Protocol-relative and relative URLs are refused at both instrument and redirect time.
