...
import { fireWebhook, isAllowedWebhookUrl, webhookPayload } from "./webhook";
...
function validateCreate(body: CreateMessageBody): string | null {
  ...
  if (body.webhook_url != null) {
    if (typeof body.webhook_url !== "string" || !isAllowedWebhookUrl(body.webhook_url)) {
...
/** Cloudflare sets CF-Connecting-IP. Do not trust X-Forwarded-For (clients can spoof it). */
